...
def read_file(file, sep, header, out_dir, out_file_name, index_col="0"):
    ...
    if header == "False":
        frame = pd.read_csv(file, sep=sep, header=None, index_col=index_col).iloc[
            :, index_col:
        ]
    else:
...
def slice_file(df_file, out_dir, out_file_name, slice_size="1000"):
    ...
    pd.DataFrame(data=np.array(df.shape + (b,)), index=["row", "col", "slice"]).to_hdf(
        out_dir + out_file_name + ".sliced.h5", "attr"
    )
    pd.DataFrame(data=df.columns, index=df.columns).to_hdf(
        out_dir + out_file_name + ".sliced.h5", "cols"
    )
    pd.DataFrame(data=df.index, index=df.index).to_hdf(
        out_dir + out_file_name + ".sliced.h5", "rows"
    )
...
def patch_file(df_file, out_dir, out_file_name):
    ...
    pd.DataFrame(data=np.array(df.shape + (1,)), index=["row", "col", "slice"]).to_hdf(
        out_dir + out_file_name + ".whole.h5", "attr"
    )
    pd.DataFrame(data=df.columns, index=df.columns).to_hdf(
        out_dir + out_file_name + ".whole.h5", "cols"
    )
    pd.DataFrame(data=df.index, index=df.index).to_hdf(
        out_dir + out_file_name + ".whole.h5", "rows"
    )
...
def merge_mi_mats(in_dir, in_common_name, n_slice, out_dir, out_file_name):
    ...
    for i in range(n_slice):
        ...
        for j in range(i):
            ...
            row_cols.append(
                pd.HDFStore(in_dir + in_common_name + "_" + key + ".h5.tmp")[key].T
            )
        for j in range(i, n_slice):
            idx = int(i * n_slice + j - (i * (i + 1)) / 2)
            key = "mi_" + str(idx).zfill(digit)
            row_cols.append(
                pd.HDFStore(in_dir + in_common_name + "_" + key + ".h5.tmp")[key]
            )
...
def norm_mi_mat(in_mat_file, out_dir, out_file_name):
    ...
    if in_mat_file == out_dir + out_file_name + "_mi.h5":
        ...
        (df / np.sqrt(np.matmul(diag.T, diag))).to_hdf(
            out_dir + out_file_name + "_mi.h5", "norm_mi"
        )
    hdf.close()
...
def hclust(fig_num, in_mat_file, out_dir, out_file_name, plot="True"):
    ...
    if plot == "True":
        heatmap(
            fig_num, df, linkage_, hc_matrix, out_dir, out_file_name, "hclust", "True"
        )
...
def scatter2(fig_num, data, out_dir, out_file_name, marker_size=20, marker="o"):
    ...
    plt.savefig(
        out_dir + out_file_name + "_clust_k" + str(len(lab)) + ".pdf",
        bbox_inches="tight",
    )


def tsne(
    fig_num, data, max_dim, out_dir, out_file_name, tag, key, perplexity=30, plot="True"
):
    tsne_ = manifold.TSNE(
        n_components=2,
        n_iter=5000,
    ...
    pd.DataFrame(data=Y_tsne, index=data.index, columns=["X", "Y"]).to_hdf(
        out_dir + out_file_name + "_clust.h5", key
    )
    if plot == "True":
...
def mds(
    fig_num, in_mat_file, max_dim, out_dir, out_file_name, perplexity=30, plot="True"
):
    hdf = pd.HDFStore(in_mat_file)
    df = 1 - hdf["norm_mi"]
    hdf.close()
...
def kmeans(in_mat_file, transformation, n_clusters, dim, out_dir, out_file_name):
    ...
    km_res = pd.DataFrame(
        data=np.transpose(km.fit_predict(df.iloc[:, 0:dim])),
        index=df.index,
        columns=["label"],
    )
    ...
    if agg != None:
        ...
        if os.path.exists(out_file):
            ...
            if re_trans != "None" and re_trans != "False" and max_dim > 0:
                ...
                hdf.close()
                perplexity = np.min(
                    [int(re_trans), np.max(cclust.groupby(["label"]).size())]
                )
                tsne(
                    None,
                    Y,
        ...
        hdf = pd.HDFStore(out_file)
        hdf["cclust"].loc[:, ["X", "Y", "label"]].to_csv(
            out_dir + out_file_name + ".ggplot.txt", sep="\t"
        )
        hdf.close()
...
def plot(fig_num, in_file, out_dir, out_file_name, hclust="False"):
    ...
    if cclust is not None and hclust == "True":
        ...
        heatmap2(
            fig_num + 2, mem, cclust.loc[:, "label"], out_dir, out_file_name, "clusts"
        )
...
def retransform(transformation, max_dim, out_dir, out_file_name, re_trans=150):
    ...
    if os.path.exists(in_file):
        shutil.copyfile(
            out_dir + out_file_name + "_clust.h5",
            out_dir + ".tmp/" + out_file_name + "_re" + str(re_trans) + "_clust.h5",
        )
        in_file = (
            out_dir + ".tmp/" + out_file_name + "_re" + str(re_trans) + "_clust.h5"
        )
        ...
        if cclust is not None and Y is not None:
            ...
            tsne(
                ...
                max_dim,
                out_dir + ".tmp/",
                out_file_name + "_re" + str(re_trans),
                None,
                transformation + "-tsne-re",
                perplexity,
                "False",
            )
            hdf = pd.HDFStore(in_file)
            ...
            hdf.close()
            reclust = pd.concat([cclust, Y_tsne.loc[cclust.index, :]], axis=1).loc[
                :, ["X", "Y", "label"]
            ]
            reclust.to_hdf(
                out_dir + out_file_name + "_re" + str(re_trans) + "_clust.h5", "reclust"
            )
            reclust.to_csv(
                out_dir + out_file_name + "_reclust_re" + str(re_trans) + ".ggplot.txt",
                sep="\t",
            )
...
def preclust(in_file, transformation, out_dir, out_file_name):
    ...
    if transformation != "lpca":
        ...
        shutil.copyfile(
            in_path + in_file_name + "_" + transformation + ".pdf",
            out_dir + out_file_name + "_" + transformation + ".pdf",
        )
        ...
        shutil.copyfile(
            in_path + in_file_name + "_lpl.pdf", out_dir + out_file_name + "_lpl.pdf"
        )
        shutil.copyfile(
            in_path + in_file_name + "_pca.pdf", out_dir + out_file_name + "_pca.pdf"
        )
