...
def setup(args):
	parser = argparse.ArgumentParser(description='Mutual Information-based Clustering Analysis.')
	...
	if args_.mode == 'Clust':
		...
		parser.add_argument('--host', default='LOCAL', help='Computation host of the jobs [LOCAL | LSF] (default: LSF)')
...
def setup_directory(out_dir, project_name, method, ks):
	...
	script_path = []
	for k in ks:
		mica_out_path.append(out_dir + 'MICA_' + project_name + '_' + method + '_' + str(k) + '/')
		log_path.append(mica_out_path[-1] + 'MICA_log/')
		out_path.append(mica_out_path[-1] + 'MICA_out/')
		script_path.append(mica_out_path[-1] + 'MICA_scripts/')
		if not os.path.exists(mica_out_path[-1]):
			os.makedirs(mica_out_path[-1])
		...
		if not os.path.exists(script_path[-1]):
			os.makedirs(script_path[-1])
	return [mica_out_path, log_path, out_path, script_path]
...
def hclust(args, paths):
	...
	for i in range(len(paths[2])):
		...
		if not os.path.exists(path_tmp):
			exit()
		script = 'hclust.py ' + ' ' + str(fig_num) + ' ' + path_tmp + args.dist_file.split('/')[-1] + ' ' + paths[2][i] + ' ' + args.outfilename + ' '
...
def transform(args, paths):
	...
	for i in range(len(paths[2])):
		...
		if not os.path.exists(path_tmp):
			exit()
		script = 'transform.py ' + ' ' + args.transformation + ' ' + str(fig_num) + ' ' + path_tmp + args.dist_file.split('/')[-1] + ' ' + str(args.max_dim) + ' ' + paths[2][i] + ' ' + args.outfilename + ' ' + str(args.perplexity) + ' ' + args.preclust + ' '
		...
		for j in range(args.bootstrap):
			...
			for k in range(dim):
				if args.transformation == 'MDS':
					script = 'kmeans.py ' + ' ' + path_tmp + args.outfilename + '_clust.h5.tmp.' + str(j * dim + k) + ' mds ' + str(args.k[i]) + ' ' + str(k + args.min_dim) + ' ' + path_tmp + ' ' + args.outfilename + '_kmeans.h5.tmp.' + str(j * dim + k) + ' '
					if args.host == 'LOCAL':
						script += '&'
					out_4.write(script + '\n')
				elif args.transformation == 'PCA':
					script = 'kmeans.py ' + ' ' + path_tmp + args.outfilename + '_clust.h5.tmp.' + str(j * dim + k) + ' pca ' + str(args.k[i]) + ' ' + str(k + args.min_dim) + ' ' + path_tmp + ' ' + args.outfilename + '_kmeans.h5.tmp.' + str(j * dim + k) + ' '
					if args.host == 'LOCAL':
						script += '&'
					out_4.write(script + '\n')
				elif args.transformation == 'LPL':
					script = 'kmeans.py ' + ' ' + path_tmp + args.outfilename + '_clust.h5.tmp.' + str(j * dim + k) + ' lpl ' + str(args.k[i]) + ' ' + str(k + args.min_dim) + ' ' + path_tmp + ' ' + args.outfilename + '_kmeans.h5.tmp.' + str(j * dim + k) + ' '
					if args.host == 'LOCAL':
						script += '&'
					out_4.write(script + '\n')
					...
					if args.host == 'LOCAL':
						script += '&'
					out_4.write(script + '\n')
					script = 'kmeans.py ' + ' ' + path_tmp + args.outfilename + '_clust.h5.tmp.' + str(iterations + j * dim + k) + ' lpl ' + str(args.k[i]) + ' ' + str(k + args.min_dim) + ' ' + path_tmp + ' ' + args.outfilename + '_kmeans.h5.tmp.' + str(iterations + j * dim + k) + ' '
					if args.host == 'LOCAL':
						script += '&'
					out_4.write(script + '\n')
		out_3.close()
...
def cclust(args, paths):
	...
	for i in range(len(paths[2])):
		...
		if not os.path.exists(path_tmp):
			exit()
		script = 'cclust.py ' + ' ' + str(fig_num) + ' ' + paths[2][i] + args.outfilename + '_clust.h5 ' + path_tmp + ' ' + str(args.k[i]) + ' ' + args.project_name + ' kmeans.h5.tmp ' + args.transformation.lower() + ' ' + paths[2][i] + ' ' + args.outfilename + ' ' + str(args.max_dim) + ' ' + str(args.retransformation) + ' '
...
def ggplot(args, paths):
	for i in range(len(paths[2])):
		script = 'ggplot.cc.r ' + ' ' + paths[2][i] + args.outfilename + '.ggplot.txt 1 5 ' + args.outfilename + ' ' + paths[2][i] + args.outfilename + '_clust_k' + str(args.k[i]) + '.rplot.pdf '
...
def reduce_clust(args, paths):
	...
	if args.host == 'LSF':
		...
		out_0.write(script)
		script = 'psub -K -P ' + args.project_name + ' -J ' + args.project_name + '_MICA_Transform.1 -q ' + args.queue + ' -M ' + str(args.resource[5]) + ' -i ' + path_tmp + '03_Share_' + args.project_name + '_' + args.transformation.lower() + '.sh -oo ' + path_tmp_log + args.project_name + '_MICA_Transform.1.%J.%I.out -eo ' + path_tmp_log + args.project_name + '_MICA_Transform.0.%J.%I.err \n'
		out_0.write(script)
...
def retransform(args, paths):
	...
	for i in range(len(paths[2])):
		...
		for j in range(len(args.retransformation)):
			script = 'retransform.py ' + ' ' + str(fig_num) + ' ' + paths[2][i] + args.outfilename + '_clust.h5 ' + args.transformation.lower() + ' ' + str(args.max_dim) + ' ' + paths[2][i] + ' ' + args.outfilename + ' ' + str(args.retransformation[j]) + ' '
			out_7.write(script + '\n')
			fig_num += 1
		out_7.close()
...
def reggplot(args, paths):
	for i in range(len(paths[2])):
		...
		for j in range(len(args.retransformation)):
			script = 'ggplot.cc.r ' + ' ' + paths[2][i] + args.outfilename + '_reclust_re' + str(args.retransformation[j]) + '.ggplot.txt 1 5 ' + args.outfilename + ' ' + paths[2][i] + args.outfilename + '_re' + str(args.retransformation[j]) + '_clust_k' + str(args.k[i]) + '.rplot.pdf '
			out_8.write(script + '\n')
		out_8.close()
