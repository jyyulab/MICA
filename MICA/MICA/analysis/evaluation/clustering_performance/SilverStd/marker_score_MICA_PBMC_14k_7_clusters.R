...
# cluster.res <- read.table(file = "/Users/lding/St. Jude Children's Research Hospital/Yu, Jiyang - scMINER/Manuscript/_Combine/v1/FigureS3/clustering_UMAP_euclidean_24_2.23.txt",
#                           sep = '\t',
#                           header = TRUE,
#                           stringsAsFactors = F)
...
colors <- c( '#00BFC4', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3', 'C3C3C3')
MICAplot(input_eset = eset.14k,
         X="X", Y="Y", # which meta variable was treated as x or y coordinates
         color_by = "trueLabel", colors = colors, pct = 0.5)

colors <- c('#C3C3C3', '#CD9600', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3', '#C3C3C3')
MICAplot(input_eset = eset.14k,
         X="X", Y="Y", # which meta variable was treated as x or y coordinates
         color_by = "trueLabel", colors = colors, pct = 0.5)
...
library(pdfCluster)
adj.rand.index(pData(eset.14k)$trueLabel, 
               pData(eset.14k)$cellType)
...
pdf(file = "/Users/lding/Desktop/Heatmap_marker_score_Adam_markers_1.82212.pdf", 
    height=10, width=10)

h.mat <- t(input[,-1])
h.mat <- h.mat[,c(3, 1, 2, 5, 6, 7, 4)]

Heatmap(as.matrix(h.mat),
...
library(Seurat)
pbmc <- readRDS(file = "/Users/lding/Documents/MICA/Datasets/HPC/SilverStd/PBMC_20k/pbmc_20k_seurat_filter_to_14k.rds")
...
true.label.14k$Seurat.cluster <- pbmc@meta.data[rownames(pbmc@meta.data) %in% cluster.res$ID,]$seurat_clusters
...
test.factor.MICA   <- factor(true.label.14k$MICA.cluster, levels=c(1,2,3,4,5,6,7), 
                             labels=c('M.Monocyte', 'M.CD8NaiveCTL', 'M.NK', 'M.Bcell', 'M.CD4TReg', 'M.CD4TCM', 'M.CD4NaiveT'))
test.factor.Seurat <- factor(true.label.14k$Seurat.cluster, levels=c(0,1,2,3,4,5,6,7), 
                             labels=c('S.CD4TCM', 'S.CD4NaiveT', 'S.NK', 'S.Bcell1',  'S.Monocyte1',  'S.CD8NaiveCTL', 'S.Bcell2', 'S.Monocyte2'))
...
test.factor.MICA <- factor(true.label.14k$MICA.cluster, levels=c(1,2,3,4,5,6,7,8), 
                           labels=c('M.Monocyte', 'M.CD8NaiveCTL', 'M.Bcell1', 'M.CD4TReg', 'M.NK', 'M.Bcell2', 'M.CD4NaiveT', 'M.CD4TCM'))
test.factor.Seurat <- factor(true.label.14k$Seurat.cluster, levels=c(0,1,2,3,4,5,6,7), 
                             labels=c('S.CD4TCM', 'S.CD4NaiveT', 'S.NK', 'S.Bcell1',  'S.Monocyte1',  'S.CD8NaiveCTL', 'S.Bcell2', 'S.Monocyte2'))
test.factor.true.label <- as.factor(true.label.14k$label)
test.factor.true.label <- factor(test.factor.true.label, levels=levels(test.factor.true.label), 
...
true.label.14k.refined$MICA.cluster <- factor(true.label.14k.refined$MICA.cluster, 
                                              ...
                                              labels=c('Monocyte', 'CD8NaiveCTL', 'Bcell1', 'CD4TReg', 'NK', 'Bcell2', 'CD4NaiveT', 'CD4TCM'))
...
true.label.14k.refined$Seurat.cluster <- factor(true.label.14k.refined$Seurat.cluster, 
                                                ...
                                                labels = c('CD4TCM', 'CD4NaiveT', 'NK', 'Bcell1', 'Monocyte1', 'CD8NaiveCTL', 'Bcell2', 'Monocyte2'))
...
cluster.res.true.label <- merge(cluster.res, true.label.20k, by.x = 'ID', by.y = 'cell')
cls <- cluster.res.true.label[cluster.res.true.label$label.x == '5',]
cls
nrow(cls[cls$label.y == 'CD4+/CD25 T Reg',]) / nrow(cls)
