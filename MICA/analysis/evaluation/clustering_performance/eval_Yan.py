...
def create_cmds():
    for dim in range(8, 100, 4):
        yan = '{}/datasets/GoldernStd/yan/Yan_MICA_input.txt'.format(mica_data_path)
        yan_out = '{}/outputs/GoldernStd/Yan/MICA_GE/{}'.format(mica_data_path, dim)

        if not os.path.isdir(yan_out):
            os.makedirs(yan_out)
        cmd = 'mica -i {} -o {} -d {} -s 0.0 -ir 0.4 -ar 2.0 -ss 0.2'.format(yan, yan_out, dim)
        print(cmd)
...
def summary():
    ...
    true_label_file = '{}/datasets/GoldernStd/yan/yan_true_label.txt'.format(mica_data_path)
