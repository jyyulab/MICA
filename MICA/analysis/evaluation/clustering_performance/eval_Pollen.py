...
def create_cmds():
    for dim in range(8, 100, 4):
        input_file = '{}/datasets/GoldernStd/pollen/Pollen_MICA_input.txt'.format(mica_data_path)
        out_dir = '{}/outputs/GoldernStd/Pollen/MICA_GE/{}'.format(mica_data_path, dim)

        if not os.path.isdir(out_dir):
            os.makedirs(out_dir)
        cmd = 'mica -i {} -o {} -d {} -s 0.0 -ir 0.2 -ar 0.4 -ss 0.2'.format(input_file, out_dir, dim)
        print(cmd)
...
def summary():
    ...
    true_label_file = '{}/datasets/GoldernStd/pollen/pollen_true_label.txt'.format(mica_data_path)
    true_label = pd.read_csv(true_label_file, delimiter='\t', header=0)
    # print(true_label)

    with open('{}/outputs/summary.txt'.format(mica_data_path), 'w') as fout:
        for dim in range(8, 100, 4):
            yan_out = '{}/outputs/GoldernStd/Pollen/MICA_GE/{}'.format(mica_data_path, dim)
            print(dim)
            for reso in np.arange(0.2, 2.1, 0.2):
